...
const ReactQuillContainer = ({ description, setDescription }: any) => {
	...
	const imageHandler2 = useCallback(() => {
		...
		input.onchange = async () => {
			...
			if (input.files) {
				...
				console.log(urlPreviewList);

				console.log(`mediaPreview is ${mediaPreview}`);
				setUrlPreviewList([...urlPreviewList, mediaPreview]);
			}
	...
	return (
		<>
			<ReactQuill
				...
				formats={formats}
				value={'<p>asfdsadf</p>'}
				onChange={(content: any) => console.log(`content is ${content}`)}
			/>
			<div style={{ height: '50px', width: '100%', backgroundColor: 'red' }}>
				{urlPreviewList.map((item: string, idx: number) => {
					return (
						<img
							key={`urlPreview_${idx}`}
							src={item}
