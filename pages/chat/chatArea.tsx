...
const ChatPage = () => {
	const items = [
		...
		{
			content: '링크를 전송 시 채팅영역에 해당 링크의 메타데이터가 표시되며 이미지 전송 시 채팅영역에 이미지가 표시됩니다.',
		},
		{
			content: '이미지 클릭 시 풀 사이즈로 볼 수 있는 모달이 나오며 링크 클릭 시 해당 사이트로 이동이 가능합니다.',
		},
		{
			content: (
				<p>
					기본적인 텍스트 붙여넣기 기능이 제공되며 <u>이미지 또한 붙여넣기가</u> 가능합니다.
				</p>
			),
		},
		{
		...
		{
			content: `일주일 치 채팅내역을 보여주고 위로 끝까지 스크롤 했을 때 이전 채팅 내역을 가져옵니다 (일주일 단위)`,
		},
		{
		...
		<>
			<main style={{ height: '100%', position: 'relative' }}>
				<Box id="chatAreaBox1" boxType="primary" textAlign="center">
					<h3 style={{ fontWeight: 'bold' }}>채팅하고픈 상대를 클릭 시 상대방과 채팅할 수 있습니다.</h3>
				</Box>
				...
				<Box id="chatAreaBox2" boxType="basic">
					...
					<List id="sampleList1" listType="buletted" verticalAlign="middle" items={items} />
				</Box>
			</main>
		</>
	);
